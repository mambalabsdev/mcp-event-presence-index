...
const TOOL_NAME = "map_company_event_presence";
const ACTOR_ID = "WLhMy8fMDgsxdYxv5";
...
const ACTOR_INPUTS = ["company_name", "domain", "include_own_events", "max_queries", "skipCache", "years"];
...
test("package identity matches the locked naming convention", () => {
  ...
  assert.deepEqual(key, ["mamba-event-presence-index"]);
  assert.deepEqual(mcp.mcpServers[key[0]].args, ["-y", pkg.name]);
  assert.equal(pkg.name, "@mambalabsdev/mcp-event-presence-index");
  assert.equal(pkg.mcpName, "com.mambabuilt/mcp-event-presence-index");
});
